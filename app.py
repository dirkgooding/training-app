...
with tab_plan:
    ...
    new_w = st.number_input(
        ...
        min_value=1, 
        max_value=12, 
        value=st.session_state.cycle_weeks
    )
    ...
    for d_key in list(st.session_state.my_plan.keys()):
        with st.expander(f"Bearbeite: {d_key}", expanded=True):
            ...
            for n in names:
                ...
                for e in cur_exs:
                    if e["name"] == n:
                        if isinstance(e["sets"], list):
                            o_sets = e["sets"]
                        else:
                            o_sets = [e["sets"]] * st.session_state.cycle_weeks
                ...
                for w in range(st.session_state.cycle_weeks):
                    ...
                    s_v = w_cols[w].number_input(
                        f"W{w+1}", 
                        1, 15, int(v), 
                        key=f"s_{d_key}_{n}_{w}"
                    )
                    n_sets.append(s_v)
            ...
            if c_s.button("Speichern", key=f"save_{d_key}"):
                if new_name != d_key:
                    st.session_state.my_plan.pop(d_key)
                st.session_state.my_plan[new_name] = upd_data
                st.rerun()
            if c_d.button("🗑️ Tag löschen", key=f"del_{d_key}"):
    ...
    if st.button("➕ Neuen Trainingstag hinzufügen"):
        st.session_state.my_plan["Neuer Tag"] = [
            {"name": "Neue Übung", "sets": [3] * st.session_state.cycle_weeks}
        ]
        st.rerun()

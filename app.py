...
with tab_train:
    ...
    if selected_day in st.session_state.my_plan:
        for i, ex in enumerate(st.session_state.my_plan[selected_day]):
            ...
            with c_n2: st.text_input(f"Note {ex['name']}", key=f"note_{ex['name']}_{w_label}_{selected_day}")
...
with tab_plan:
    ...
    for d_key in list(st.session_state.my_plan.keys()):
        with st.expander(f"Edit Day: {d_key}", expanded=True):
            ...
            new_dn = col_d1.text_input("Rename Day:", value=d_key, key=f"ren_{d_key}")
            if col_d2.button("Delete Day", key=f"btn_del_{d_key}"):
            ...
            for n in names:
                ...
                if p_type == "Expert Matrix":
                    ...
                    if p_type == "Double Progression":
                        ...
                        label = "Goal (Seconds)" if "Time" in p_type else "Goal (Reps)"
                        g_r = c2.number_input(label, 1, 300, int(o_prog.get("glob_reps", 10)), key=f"gr_{d_key}_{n}")
                ...
                with st.expander("⚙️ Logic & Increments"):
                    l1, l2 = st.columns(2)
                    if "Weight" in p_type or "Double" in p_type or "Expert" in p_type: 

...
with tab_data:
    ...
    if uploaded_csv is not None:
        if st.button("Import bestätigen und Plan aktualisieren"):
            try:
                ...
                if not df_import.empty:
                    ...
                    # Zyklus-Wochen und Plan-Struktur neu aufbauen
                    max_w = 1
                    for w_str in df_import['Woche'].unique():
                        try:
                            num = int(str(w_str).replace("Woche ", "").strip())
                            if num > max_w: max_w = num
                        except: pass
                    st.session_state.cycle_weeks = max_w
                    
                    new_plan = {}
                    for tag in df_import['Tag'].unique():
                        new_plan[tag] = []
                        df_tag = df_import[df_import['Tag'] == tag]
                        for ex in df_tag['Übung'].unique():
                            df_ex = df_tag[df_tag['Übung'] == ex]
                            sets_list = []
                            for w in range(1, max_w + 1):
                                df_w = df_ex[df_ex['Woche'] == f"Woche {w}"]
                                if not df_w.empty:
                                    sets_list.append(int(df_w['Satz'].max()))
                                else:
                                    sets_list.append(sets_list[-1] if sets_list else 3)
                            new_plan[tag].append({"name": ex, "sets": sets_list})
                    
                    st.session_state.my_plan = new_plan
                st.success("Daten und Plan erfolgreich importiert!")
                st.rerun()

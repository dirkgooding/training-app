    ...
    for d_key in list(st.session_state.my_plan.keys()):
        with st.expander(f"Edit Day: {d_key}", expanded=True):
            ...
            for n in names:
                ...
                p_type = st.selectbox("Choose your progression model", prog_options, index=prog_options.index(o_prog["type"]) if o_prog["type"] in prog_options else 0, key=f"ptype_{d_key}_{
